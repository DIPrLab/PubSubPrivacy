...
```bash
# Local, use all cores, default sharding (dataset x clamp_mode = 12 shards),
...
# MAXIMALLY parallel: finer shards (dataset x clamp x experiment, ~108 shards)
# for a big node pool, 32 concurrent.  run_cluster.sh / submit_coeus.sh run the
# grid shards in PHASE 1 and the experiment shards in PHASE 2 automatically
# (F/G/H consume the grid via --use-grid-config); no manual two-step needed.
...
| `CLAMP_MODES` | `static dp_released` | clamp modes to run |
| `EXPERIMENTS` | `sweep A B C D F G H` | experiments for `dataset-clamp-exp` |
